...
public class RolesRunner implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        if (!rolesService.existsByRoleType(Roles.ROLES_BARMAN)) {
            rolesService.create(new Roles(Roles.ROLES_BARMAN));
            System.out.println("Ruolo BARMAN inserito");
        } else {
            System.out.println("Ruolo BARMAN già presente");
        }
    }
}
